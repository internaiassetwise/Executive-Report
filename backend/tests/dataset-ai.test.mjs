...
test('missing keys, provider failures and ungrounded output leave deterministic analysis available', async () => {
  ...
  for (const body of [
    { ...valid, summary: 'ค่าเฉลี่ย 99999' },
    { ...valid, insights: [{ ...valid.insights[0], description: 'ค่าเฉลี่ย -12' }] },
    { ...valid, insights: [{ ...valid.insights[0], evidence_ids: ['EV-999'] }] },
    { ...valid, recommendations: [{ text: 'ค่าเฉลี่ย 99999', evidence_ids: ['EV-001'] }] },
  ]) {
