...
export function AccessGate({ onUnlocked }: { onUnlocked: () => void }) {
  ...
  return <section className="data-upload-panel access-gate" aria-labelledby="access-title">
    ...
    <form onSubmit={event => void submit(event)}>
      ...
      <input id="access-password" type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} aria-invalid={Boolean(error)} aria-describedby={error ? 'access-error' : undefined} autoFocus />
